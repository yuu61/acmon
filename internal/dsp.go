...
// removeDC は DC 成分（窓内平均）を引いた信号を返す。
func removeDC(raw []float64) []float64 {
	dc := mean(raw)
...
func zeroCrossFreq(s, tsec []float64) float64 {
	...
	first, last, n := collectCrossings(s, tsec)
	if n < 2 {
		return 0
	}
...
// collectCrossings は立ち上がりゼロクロスを走査し、最初と最後の補間時刻および
// 検出本数を返す。各クロスは s[i-1]→s[i] の間で線形補間する。
func collectCrossings(s, tsec []float64) (float64, float64, int) {
...
func spectrum(fft *fourier.FFT, windowed []float64, windowGain float64) []float64 {
